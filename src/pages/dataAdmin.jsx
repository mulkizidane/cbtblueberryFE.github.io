...
const DataAdminPage = () => {
    ...
    return (
            ...
            <div className="flex flex-col w-full pt-20 px-7">
                ...
                <div className="flex gap-2">
                    <div className={`bg-primary ${userId == 'siswa' ? 'w-full' : 'w-[70%]'} rounded-xl overflow-hidden shadow-multiple`}>
                        <Heading title={'Data Administrator'}/>
                        <div className="w-full flex justify-between py-2 pr-2">
                            <div className="flex items-center gap-2 pl-2 text-sm font-semibold">
                                ...
                                <select onChange={handleOption} name="entries" id="entries" className="rounded-full bg-transparent w-16 pl-4">
                                    {
                                        data?.map((dt,index) => (
                                            <option key={dt.id} value={index+1}>{index+1}</option>
                                        ))
                                    }
                                </select>
                                <h1>entries</h1>
                    ...
                    onClick={
                        (e) => handleAddData(
                            e,
                            `http://localhost:5000/${userId}/administrator`, 
                            `http://localhost:5000/${userId}/administrator/${dataId}`, 
                            newData, 
                            setDataId, 
                            isEdit,
                            setErrMsg,
        ...
        onClick={() => handleImportFile(file, `http://localhost:5000/${userId}/administrators/import`, setShowModal, setErrMsg)}
        showModal={showModal}
